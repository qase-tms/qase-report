...
## Current Milestone: v1.8 CLI & NPM Package

**Goal:** Создать CLI утилиту для запуска и генерации отчётов, публикация в NPM.

**Target features:**
- `qase-report open <path>` — запуск сервера и открытие браузера
- `qase-report generate <path>` — генерация static HTML отчёта
- Автосохранение истории при открытии прогона
- Опция `--history <file>` для указания пути к истории
- NPM package для глобальной установки
